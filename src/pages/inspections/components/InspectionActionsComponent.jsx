  ...
  renderPopoverContent = () => {
    ...
    return (
      <Row>
        ...
        {((hasPermission(PERMISSIONS.auditorBase) || hasPermission(PERMISSIONS.auditorInitiator)) && _.isEqual(checklistData.auditor.email, userInfo.sub)) &&
        <Row style={{ marginBottom: 10 }}>
        ...
        {(checklistData.status.id === 2) && <Row style={{ marginBottom: 10 }}>
          <Col span={24} key={`col_export_button__${checklistData.id}`}>
            <Button
              icon="download"
              style={{ width: '100%' }}
              onClick={() => this.props.handleExportTemplate(checklistData)}
              key={`export_button_${checklistData.id}`}
            >
              {translate('export_button_to_pdf_name')}
            </Button>
          </Col>
        </Row>}
