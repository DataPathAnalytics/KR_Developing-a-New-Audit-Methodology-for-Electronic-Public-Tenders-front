...
import { Modal, Form, Input, Switch, Tree, Radio } from 'antd'
...
class EditAuditorForm extends Component {
  ...
  setInitialValues = () => {
    const { form, auditorData, permissions } = this.props
    form.setFieldsValue({
      id: auditorData.id,
  ...
  preparePermissionsRadio = (permissionData) => {
    ...
    _.forEach(this.props.permissions, (permission) => {
      permissionComponent.push(<Radio style={radioStyle} value={permission.id}>{permission[this.props.permissionKey]}</Radio>)
      // permissionData[0].children.push({
