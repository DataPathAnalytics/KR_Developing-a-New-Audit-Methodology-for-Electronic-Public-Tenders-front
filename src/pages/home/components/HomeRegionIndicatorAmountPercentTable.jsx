...
import { DASHBOARD_REGION_INDICATOR_AMOUNT_PERCENT_COLUMNS } from '../HomePageConstants'

class HomeRegionIndicatorAmountPercentTable extends Component {
  componentDidMount() {
  ...
  prepareTableColumns = () => {
    ...
    let tableColumns = []
    let indicatorColumn = DASHBOARD_REGION_INDICATOR_AMOUNT_PERCENT_COLUMNS.mainColumn
    indicatorColumn.title = translate(indicatorColumn.translateKey)
    tableColumns.push(indicatorColumn)

    _.forEach(regions, (item) => {
      let tableColumnTemplate = _.cloneDeep(DASHBOARD_REGION_INDICATOR_AMOUNT_PERCENT_COLUMNS.childColumn)
  ...
  prepareTableData = () => {
    const { translate, dashboardBaseRegionIndicatorAmountPercent, allMappings: { indicators, regions }, indicatorsKey } = this.props
    let tableData = []
    let dataTemplate = {}
    ...
    let chainedData = _.chain(_.cloneDeep(dashboardBaseRegionIndicatorAmountPercent))
      .groupBy('indicatorId')
      .map((values, key) => {
        return {
  ...
  render() {
    const { translate, dashboardBaseRegionIndicatorAmountPercent, dashboardBaseRegionIndicatorAmountPercentIsFetching, allMappings: { indicators } } = this.props

    // if (_.isEmpty(dashboardBaseRegionIndicatorAmountPercent)) {
    //   return null
    // }

    return (
      <Spin spinning={dashboardBaseRegionIndicatorAmountPercentIsFetching} size="large">
        <Table
          useFixedHeader
          rowKey='id'
          ...
          title={() => <div className="table-title-centered"><b>
            {translate('chart_title_procedures_distribution_by_indicators_and_regions')} ({translate('chart_status_card_percentage_of_the_total_procedures_sum')})</b>
          </div>}
...
function mapStateToProps({
  ...
  return {
    dashboardBaseRegionIndicatorAmountPercent: homeStore.dashboardBaseRegionIndicatorAmountPercent,
    dashboardBaseRegionIndicatorAmountPercentIsFetching: homeStore.dashboardBaseRegionIndicatorAmountPercentIsFetching,
...
export default connect(
  mapStateToProps,
)(withTranslate(HomeRegionIndicatorAmountPercentTable))
