...
import { DASHBOARD_REGION_INDICATOR_COUNT_COLUMNS } from '../HomePageConstants'

class HomeRegionIndicatorCountTable extends Component {
  componentDidMount() {
  ...
  prepareTableColumns = () => {
    ...
    let tableColumns = []
    let indicatorColumn = DASHBOARD_REGION_INDICATOR_COUNT_COLUMNS.mainColumn
    indicatorColumn.title = translate(indicatorColumn.translateKey)
    tableColumns.push(indicatorColumn)

    _.forEach(regions, (item) => {
      let tableColumnTemplate = _.cloneDeep(DASHBOARD_REGION_INDICATOR_COUNT_COLUMNS.childColumn)
  ...
  prepareTableData = () => {
    const { translate, dashboardBaseRegionIndicatorCount, allMappings: { indicators, regions }, indicatorsKey } = this.props
    let tableData = []
    let dataTemplate = {}
    ...
    let chainedData = _.chain(_.cloneDeep(dashboardBaseRegionIndicatorCount))
      .groupBy('indicatorId')
      .map((values, key) => {
        return {
  ...
  render() {
    const { translate, dashboardBaseRegionIndicatorCount, dashboardBaseRegionIndicatorCountIsFetching, allMappings: { indicators } } = this.props

    // if (_.isEmpty(dashboardBaseRegionIndicatorCount)) {
    //   return null
    // }

    return (
      <Spin spinning={dashboardBaseRegionIndicatorCountIsFetching} size="large">
        <Table
          useFixedHeader
          rowKey="id"
          ...
          pagination={false}
          title={() => <div className="table-title-centered">
            <b>{translate('chart_title_procedures_distribution_by_indicators_and_regions')} ({translate('chart_tabs_amount')})</b>
          </div>}
...
function mapStateToProps({
  ...
  return {
    dashboardBaseRegionIndicatorCount: homeStore.dashboardBaseRegionIndicatorCount,
    dashboardBaseRegionIndicatorCountIsFetching: homeStore.dashboardBaseRegionIndicatorCountIsFetching,
...
export default connect(
  mapStateToProps,
)(withTranslate(HomeRegionIndicatorCountTable))
