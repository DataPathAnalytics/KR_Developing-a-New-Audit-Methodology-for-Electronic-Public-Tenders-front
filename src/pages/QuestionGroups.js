...
import { Collapse, Card, Row, Col, Input, Button, Tooltip, Select } from 'antd'
...
class QuestionGroups extends Component {
  ...
  render() {
    ...
    // let openedItems = categories ? categories.map((category) => (category.id.toString())) : []
    let openedItems = []
    for (let i = 0; i < 100; i++) {
  ...
  createQuestion = (question, questionWithAnswer) => {
    ...
    let questionNumber = parseInt(question.tempIndex.split('_')[1], 0) + 1
      ...
      <Card bordered key={question.tempIndex}>
        <Row>
          ...
          <Col span={24}>
            ...
            <Row style={{ marginTop: 10, marginLeft: 15 }}>
              <Input.TextArea autosize={{ minRows: 1, maxRows: 10 }} placeholder={translate('option_npa')}
                              onChange={e => this.handleNPA(e, question)}
                              defaultValue={question.hasOwnProperty('npa') ? question.npa : ''} />
            </Row>
          </Col>
        </Row>
      </Card>
  ...
  handleNPA = (event, question) => {
    question.npa = event.target.value
    this.props.handleQuestion(question)
  }
}
