...
import { fetchTenderMethodIndicatorCount } from '../../../../redux/action/prioritization/PrioritizationActions'
...
import { DASHBOARD_INDICATORS_TABLE_COUNT_COLUMNS } from '../../PrioritizationConstants'

class TenderRiskTendersCountInfoByMethodTable extends Component {
  componentDidMount() {
  ...
  prepareTableColumns = () => {
    ...
    let tableColumns = _.cloneDeep(DASHBOARD_INDICATORS_TABLE_COUNT_COLUMNS)
  ...
  prepareTableData = () => {
    const { translate, dashboardTenderMethodIndicatorCount, allMappings: { indicators, procurementMethodDetails }, indicatorsKey } = this.props
    let tableData = []
    let dataTemplate = {}
    ...
    let chainedData = _.chain(_.cloneDeep(dashboardTenderMethodIndicatorCount))
      .groupBy('indicatorId')
      .map((values, key) => {
        return {
  ...
  render() {
    const { translate, dashboardTenderMethodIndicatorCount, dashboardTenderMethodIndicatorCountIsFetching, allMappings: { indicators } } = this.props

    // if (_.isEmpty(dashboardTenderMethodIndicatorCount)) {
    //   return null
    // }

    return (
      <Spin spinning={dashboardTenderMethodIndicatorCountIsFetching} size="large">
        <Table
          rowKey='id'
...
function mapStateToProps({
  ...
  return {
    dashboardTenderMethodIndicatorCount: prioritizationStore.dashboardTenderMethodIndicatorCount,
    dashboardTenderMethodIndicatorCountIsFetching: prioritizationStore.dashboardTenderMethodIndicatorCountIsFetching,
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withTranslate(TenderRiskTendersCountInfoByMethodTable))
