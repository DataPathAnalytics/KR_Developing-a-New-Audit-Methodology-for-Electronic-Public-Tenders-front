...
class PrioritizationTemplatesModal extends Component {
  ...
  render() {
    const { translate, defaultTemplates, userTemplates } = this.props
    ...
    return (
      ...
      >
        <Row>
          ...
          <Col span={24}>
            <Table
              title={() => (<strong>{translate('user_templates_table_header')}</strong>)}
              showHeader={false}
              bordered
              rowKey='id'
              size="small"
              ...
              indentSize={15}
              columns={templateTableColumns}
              dataSource={this.addTemplatePosition(userTemplates)}
              onRow={(record, rowIndex) => {
                return {
                  onClick: () => {
                    this.handleRowSelection(record, rowIndex)
